 ...
 */
@Repository
public interface FileStubRepository  {
     ...
     */
    List<FileStub> findByUserIdAndStatus(String userId, StubStatus status);
    
    /**
     * Find file stubs by status with non-null cron expression
     * @param status the status to filter by
     * @return list of file stubs with non-null cron expression
     */
    List<FileStub> findByStatusAndCronExpressionNotNull(StubStatus status);
} 